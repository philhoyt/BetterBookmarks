...
export default function Edit( { attributes, setAttributes } ) {
	const { url, title, description, image, domain } = attributes;
	...
	async function fetchPreview( fetchUrl ) {
		...
		try {
			const data = await apiFetch( {
				path: `/better-bookmarks/v1/preview?url=${ encodeURIComponent( fetchUrl ) }`,
			} );
			setAttributes( {
				url:         data.url,
				title:       data.title,
			...
			setError( err.message ?? __( 'Could not fetch preview.', 'better-bookmarks' ) );
		} finally {
			setLoading( false );
		}
	}
		...
		<div { ...blockProps }>
			<InspectorControls>
				<PanelBody title={ __( 'Link', 'better-bookmarks' ) }>
					...
					>
						{ loading ? <Spinner /> : __( 'Fetch Preview', 'better-bookmarks' ) }
					</Button>
			...
			{ ! hasCard && (
				<div className="bb-link-card-editor__placeholder">
					<input
						...
						onKeyDown={ handleKeyDown }
						placeholder={ __( 'Paste a URL and press Enter…', 'better-bookmarks' ) }
					/>
					{ loading && <Spinner /> }
					{ error && <p className="bb-editor-error">{ error }</p> }
				</div>
			...
			{ hasCard && (
				<div className="bb-link-card">
					<a className="bb-link-card__link" href={ url } target="_blank" rel="noopener noreferrer">
						{ image && (
							<div className="bb-link-card__image-wrap">
								<img className="bb-link-card__image" src={ image } alt="" />
							</div>
						) }
						<div className="bb-link-card__body">
							{ domain && <span className="bb-link-card__domain">{ domain }</span> }
							{ title && <strong className="bb-link-card__title">{ title }</strong> }
							{ description && <p className="bb-link-card__description">{ description }</p> }
						</div>
					</a>
				</div>
			) }
